...
class Settings:
    ...
    def define_settings(self) -> None:
        data = re.findall('[\d\w]+', self._indicator_string)
        algo_name, setting_words = data

        self.settings = {
            "candle_size": setting_words[0],
            "method": setting_words[-1]
        }

        func = getattr(self, f"settings_{algo_name}")
        func()
